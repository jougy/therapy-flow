...
async function verify() {
  console.log('Iniciando teste de verificação da release alfa-26.08.22-01...');
  ...
  const isVersionVisible = await desktopPage.getByText("alfa-26.08.22-01").first().isVisible();
  console.log('Versão alfa-26.08.22-01 visível no painel:', isVersionVisible);
  ...
  const tutoriaisItem = desktopPage.getByText("Sistema Completo de Tutoriais Guiados");
  console.log('Tópico "Sistema Completo de Tutoriais..." visível:', await tutoriaisItem.isVisible());

  const editorItem = desktopPage.getByText("Novo Construtor e Editor Modular de Formulários");
  console.log('Tópico "Novo Construtor e Editor..." visível:', await editorItem.isVisible());

  if (!fs.existsSync('core/screenshots')) {
    fs.mkdirSync('core/screenshots', { recursive: true });
  }

  await desktopPage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.22-01-desktop.png', fullPage: true });
  console.log('Screenshot desktop salvo em core/screenshots/novidades-alfa-26.08.22-01-desktop.png');
  ...
  await mobilePage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.22-01-mobile.png', fullPage: true });
  console.log('Screenshot mobile salvo em core/screenshots/novidades-alfa-26.08.22-01-mobile.png');

  await browser.close();
  console.log('\n✅ Validação da release alfa-26.08.22-01 concluída com sucesso!');
}
