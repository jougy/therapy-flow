...
async function verify() {
  console.log('Iniciando teste de verificação da release alfa-26.08.12-01...');
  ...
  // Click Novidades da Plataforma / Editor tab if available
  const novidadesTab = desktopPage.locator('button:visible:has-text("Novidades da Plataforma"), button:visible:has-text("Novidades")').first();
  if (await novidadesTab.isVisible()) {
    await novidadesTab.click();
    await desktopPage.waitForTimeout(1000);
  }

  const isVersionVisible = await desktopPage.getByText("alfa-26.08.12-01").first().isVisible();
  console.log('Versão alfa-26.08.12-01 visível no painel:', isVersionVisible);
  ...
  const moduleItem = desktopPage.getByText("Módulo Integrado de Assinaturas e Faturas da Clínica");
  console.log('Tópico "Módulo Integrado de Assinaturas..." visível:', await moduleItem.isVisible());

  const antiPrintItem = desktopPage.getByText("Proteção Anti-Print e Confirmação de Responsabilidade");
  console.log('Tópico "Proteção Anti-Print..." visível:', await antiPrintItem.isVisible());

  // Ensure screenshots directory exists in /core/screenshots
  if (!fs.existsSync('core/screenshots')) {
    fs.mkdirSync('core/screenshots', { recursive: true });
  }

  await desktopPage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.12-01-desktop.png', fullPage: true });
  console.log('Screenshot desktop salvo em core/screenshots/novidades-alfa-26.08.12-01-desktop.png');

  // 2. Mobile Viewport Test (375x812)
  console.log('\n--- Testando em Mobile (375x812 - iPhone X/12/13/14) ---');
  ...
  await mobilePage.goto('http://localhost:8080/platform', { waitUntil: 'networkidle' });
  ...
  await mobilePage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.12-01-mobile.png', fullPage: true });
  console.log('Screenshot mobile salvo em core/screenshots/novidades-alfa-26.08.12-01-mobile.png');

  await browser.close();
  console.log('\n✅ Validação da release alfa-26.08.12-01 concluída com sucesso!');
}
