...
async function verify() {
  console.log('Iniciando teste de verificação da release alfa-26.08.14-01...');
  ...
  const isVersionVisible = await desktopPage.getByText("alfa-26.08.14-01").first().isVisible();
  console.log('Versão alfa-26.08.14-01 visível no painel:', isVersionVisible);
  ...
  const urlsItem = desktopPage.getByText("URLs Amigáveis e Links Diretos para Pacientes");
  console.log('Tópico "URLs Amigáveis..." visível:', await urlsItem.isVisible());

  const resumoItem = desktopPage.getByText("Atalho para Resumo Clínico no Cabeçalho do Prontuário");
  console.log('Tópico "Atalho para Resumo Clínico..." visível:', await resumoItem.isVisible());

  if (!fs.existsSync('core/screenshots')) {
    fs.mkdirSync('core/screenshots', { recursive: true });
  }

  await desktopPage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.14-01-desktop.png', fullPage: true });
  console.log('Screenshot desktop salvo em core/screenshots/novidades-alfa-26.08.14-01-desktop.png');
  ...
  await mobilePage.screenshot({ path: 'core/screenshots/novidades-alfa-26.08.14-01-mobile.png', fullPage: true });
  console.log('Screenshot mobile salvo em core/screenshots/novidades-alfa-26.08.14-01-mobile.png');

  await browser.close();
  console.log('\n✅ Validação da release alfa-26.08.14-01 concluída com sucesso!');
}
