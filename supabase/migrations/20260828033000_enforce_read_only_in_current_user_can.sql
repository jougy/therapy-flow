-- Migration: 20260828033000_enforce_read_only_in_current_user_can.sql
-- Garante que current_user_can (usado em TODAS as policies de RLS do Postgres)
-- bloqueia automaticamente qualquer escrita quando a assinatura da clinica estiver expirada.
...
BEGIN
  ...
  IF _is_subscription_expired THEN
    ...
    IF _capability IN ('patients.read', 'schedule.read', 'sessions.read', 'subaccounts_analytics.read') THEN
      RETURN true;
    END IF;
    -- Qualquer escrita (patients.write, sessions.write, schedule.write, forms.manage, etc.) e bloqueada
    RETURN false;
  END IF;

  -- Se nao estiver expirado, segue as permissoes normais
