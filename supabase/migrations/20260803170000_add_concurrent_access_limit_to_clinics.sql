-- Add concurrent_access_limit column to clinics table and update list_current_user_clinics RPC

alter table public.clinics
  add column if not exists concurrent_access_limit integer;
...
as $$
  select
    ...
    coalesce(clinics.concurrent_access_limit, case when clinics.subscription_plan = 'clinic' then greatest(clinics.subaccount_limit, 2) else 1 end)::integer as clinic_concurrent_access_limit,
