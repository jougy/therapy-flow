...
BEGIN
  ...
  SELECT
    COUNT(*)::int,
    COUNT(*) FILTER (WHERE (recurring_weekdays IS NOT NULL AND array_length(recurring_weekdays, 1) > 0) OR is_recurring = true)::int,
    COALESCE(jsonb_object_agg(COALESCE(ps.status, 'ativo'), ps.cnt), '{}'::jsonb)
  INTO
    _total_patients,
    _recurring_patients,
  ...
  IF _total_patients IS NULL OR _total_patients = 0 THEN
    SELECT
      COUNT(*)::int,
      COUNT(*) FILTER (WHERE (recurring_weekdays IS NOT NULL AND array_length(recurring_weekdays, 1) > 0) OR is_recurring = true)::int
    INTO
