...
begin
  ...
  -- Evitar disparar notificação repetida de "Novo login registrado" se o mesmo dispositivo/usuário
  -- já registrou um evento de login nas últimas 12 horas.
  if _reactivated then
    select not exists (
      ...
      where target_user_id = _user_id
        ...
        and (
          (payload->>'session_key') = _session_key
          or (
            nullif(trim(_user_agent), '') is not null 
            and (payload->>'user_agent') = nullif(trim(_user_agent), '')
          )
        )
    ) into _should_notify;
